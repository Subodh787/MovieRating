...
const createOrder = (req, res) => {
    ...
    db.serialize(() => {
        ...
        items.forEach((item, index) => {
            db.get('SELECT id, price, stock_quantity FROM products WHERE id = ?', [item.productId], (err, product) => {
                if (err) {
                    db.run('ROLLBACK');
                ...
                if (processedItems === items.length) {
                    ...
                    db.run('INSERT INTO orders (user_id, total_amount, shipping_address) VALUES (?, ?, ?)', [userId, totalAmount, shippingAddress], function (err) {
                        ...
                        orderItems.forEach((orderItem) => {
                            db.run('INSERT INTO order_items (order_id, product_id, quantity, price) VALUES (?, ?, ?, ?)', [orderId, orderItem.productId, orderItem.quantity, orderItem.price], (err) => {
                                if (err) {
                                    db.run('ROLLBACK');
                                ...
                                // Update stock
                                db.run('UPDATE products SET stock_quantity = stock_quantity - ? WHERE id = ?', [orderItem.quantity, orderItem.productId], (err) => {
                                    if (err) {
                                        db.run('ROLLBACK');
